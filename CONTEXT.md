...
**Bridge**:
Контейнер с supergateway/mcpo, оборачивающий готовый stdio-only MCP-сервер в HTTP/SSE. Применяется там, где у готового MCP нет нативного HTTP/Docker-образа (Vikunja, Wger) либо образ не собран под целевую арку (Firefly III — образ только amd64).
_Avoid_: прокси, шлюз
